...
namespace Aura.Channel.Network.Sending
{
	public static partial class Send
	{
		...
		public static void Transform(Creature creature, Skill skill, bool transforming, byte TransformId)
		{
			// TODO: Add actual transform level from rank info.
			// intVal1 = look, intVal2 = titleId
			var p = new Packet(0xA41C, creature.EntityId);
			p.PutByte((byte)(transforming ? TransformId : (byte)0));
			p.PutUShort((ushort)(transforming ? 15 : 0));
			if (skill.Info.Rank < SkillRank.RA)
			{
				p.PutShort((short)(transforming ? 1 : 0));
			}

			else if (skill.Info.Rank >= SkillRank.RA && skill.Info.Rank < SkillRank.R5)
			{
				p.PutUShort((ushort)(transforming ? 2 : 0));
			}

			else if (skill.Info.Rank >= SkillRank.R5 && skill.Info.Rank < SkillRank.R1)
			{
				p.PutUShort((ushort)(transforming ? 3 : 0));
			}

			else
			{
				p.PutUShort((ushort)(transforming ? 4 : 0));
			}
			p.PutByte(1);
		...
		public static void SpreadWings(Creature creature, bool onOff)
		{
			var packet = new Packet(onOff ? Op.SpreadWingsOn : Op.SpreadWingsOff, creature.EntityId);
			creature.Region.Broadcast(packet, creature);
		}
