...
namespace Aura.Channel.Skills.Combat
{
	...
	public class SupportShotHandler : CombatSkillHandler, IInitiableSkillHandler
	{
		/// To do:
		...
		public override CombatSkillResult Use(Creature attacker, Skill skill, long targetEntityId)
		{
			...
			//	bool hit = false;
			var aimChance = attacker.GetAimPercent(attacker.Skills.Get(SkillId.RangedAttack).RankData.Var3);
			...
			if (Rnd.NextDouble() < Math.Min(aimChance / 100f, 1))
			{
				...
				attacker.Region.Broadcast(new Packet(Op.Effect, target.EntityId).PutInt(Effect.SupportShot).PutByte(1));

				System.Threading.Thread t = new System.Threading.Thread(() =>
				{
					System.Threading.Thread.Sleep(3000);
					attacker.Region.Broadcast(new Packet(Op.Effect, target.EntityId).PutInt(Effect.SupportShot).PutByte(0));
				});
