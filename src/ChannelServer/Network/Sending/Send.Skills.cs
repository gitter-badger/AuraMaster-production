...
namespace Aura.Channel.Network.Sending
{
	public static partial class Send
	{
		...
		/// <param name="skillId"></param>
		/// <param name="unkByte"></param>
		public static void SkillUseHeal(Creature creature, SkillId skillId, long targetId)
		{
			var packet = new Packet(Op.SkillUse, creature.EntityId);
			packet.PutUShort((ushort)skillId);
		...
		public static void SkillStackUpdate(Creature creature, Skill skill, byte remaining)
		{
			var packet = new Packet(Op.SkillStackUpdate, creature.EntityId);
			packet.PutByte(remaining); 
			packet.PutByte(1);
			packet.PutByte(0);
			packet.PutShort((short)skill.Info.Id);

			creature.Region.Broadcast(packet);
		}

		public static void SkillStackSet(Creature creature, Skill skill, byte stack)
		{
			var packet = new Packet(Op.SkillStackSet, creature.EntityId);
			packet.PutByte(creature.ActiveSkillStacks);
			packet.PutByte(1);
			packet.PutShort((short)creature.Skills.ActiveSkill.Info.Id);
