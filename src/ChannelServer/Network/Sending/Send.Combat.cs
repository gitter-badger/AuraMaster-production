...
namespace Aura.Channel.Network.Sending
{
	public static partial class Send
	{
		...
		public static void CombatSetAimR(Creature creature, Byte UnkByte, Byte UnkByte2, long targetId)
		{
			var packet = new Packet(Op.CombatSetAimR, creature.EntityId)
			.PutByte(UnkByte)
			.PutLong(targetId)
			.PutShort((short)creature.Skills.ActiveSkill.Info.Id)
			.PutByte(UnkByte2);

			creature.Client.Send(packet);
		}

		public static void CombatSetAimR(Creature creature,  Byte UnkByte)
		{
			var packet = new Packet(Op.CombatSetAimR, creature.EntityId)
			.PutByte(UnkByte);
