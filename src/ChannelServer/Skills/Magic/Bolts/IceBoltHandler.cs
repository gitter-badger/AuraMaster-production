...
namespace Aura.Channel.Skills.Magic.Bolts
{
	...
	public class IceBoltHandler : CombatSkillHandler, IInitiableSkillHandler
	{
		public const short UseStun = 500; 
		public const short TargetStun = 1650;
		...
		public override void Ready(Creature creature, Skill skill, Packet packet)
		{
			if (creature.ActiveSkillStacks < skill.RankData.StackMax)
			{
				...
				creature.Region.Broadcast(new Packet(Op.Effect, creature.EntityId).PutInt(Effect.StackUpdate).PutString("icebolt").PutByte(creature.ActiveSkillStacks).PutByte(0));
		...
		public override CombatSkillResult Use(Creature attacker, Skill skill, long targetEntityId)
		{
			...
			attacker.Region.Broadcast(new Packet(Op.Effect, attacker.EntityId).PutInt(Effect.UseMagic).PutString("icebolt"));
		...
		//	var targetSkill = tAction.Creature.Skills.Get(SkillId.Icebolt);
		...
		//					{
		//						//Send.SkillComplete(tAction.Creature, targetSkill.Info.Id);
		//						tAction.Creature.Skills.ActiveSkill = null;
		//						Send.SkillUseSilentCancel(tAction.Creature);
		//					}
		//					break;
