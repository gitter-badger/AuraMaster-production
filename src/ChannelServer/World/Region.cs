...
namespace Aura.Channel.World
{
	public class Region
	{
		...
		public List<Creature> GetAttackableCreaturesInRange(Entity entity, int range = 0)
		{
			if (range < 1)
				range = (int)ChannelServer.Instance.Conf.World.SightRange;

			lock (_creatures)


				return _creatures.Values.Where(a => a != entity && a is NPC && !(a as NPC).Has(CreatureStates.GoodNpc) && !a.IsDead && a.Region == entity.Region && a.GetPosition().InRange(entity.GetPosition(), range)).ToList();
		}
