...
namespace Aura.Channel.World.Entities
{
	...
	public abstract class Creature : Entity, IDisposable
	{
		...
		public ShamalaData Shamala = null;
		public RaceData ShamalaRace = null;
		...
		public void Move(Position destination, bool walking)
		{
			...
			// Server Updates
			if (this.IsMoving && !this.IsElf && this.IsPlayer)
			{
				switch (this.Skills.ActiveSkill.Info.Id)
				{
				case SkillId.RangedAttack:
				case SkillId.ArrowRevolver:
				case SkillId.ArrowRevolver2:
				case SkillId.MagnumShot:
				case SkillId.SupportShot:
				case SkillId.ElvenMagicMissile:
				case SkillId.MirageMissile:
				case SkillId.CrashShot:
				this.ResetAim();
					break;
			}
		...
		public float GetSpeed()
		{
			...
			if (this.Conditions.Has(ConditionsB.Demigod))
				speed *= 1.5f;
		...
		/// <summary>
		/// Calculates the damage for magic.
		/// </summary>
		/// <returns></returns>
		public float GetMagicDamage(Item item, float spellDamage)
		{
		...
		public double GetAimPercent(double aimBoost, double aimInitial = 0)
		{
			...
			var distance = aPos.GetDistance(tPos);

			if (!aPos.InRange(tPos, (int)(this.RightHand.OptionInfo.EffectiveRange)))		
			{
				// TODO: SoG?
				return 0;
			}

			var aimTime = (DateTime.Now - this.Temp.AimStart).TotalMilliseconds / 4.9;
			double aimRate = Math.Min(320 / (double)distance , 1);
			var aimPercent = Math.Min(aimTime * (aimRate * (aimBoost / 100)) + aimInitial, 99);
		...
		{
			if (this.Target != null)
				this.Client.Send(new Packet(Op.CombatSetAimR, this.EntityId).PutByte(0));
